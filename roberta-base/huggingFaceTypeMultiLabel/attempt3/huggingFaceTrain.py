...
# Funkcja do obliczania wag klas na podstawie zbioru treningowego
def compute_class_weights(train_dataset, num_labels):
    # Zakładamy, że kolumna "labels" to lista multi-hot wektorów (0/1) o długości num_labels
    all_labels = np.array(train_dataset["labels"])
    # Liczba przykładów
    total_samples = len(all_labels)
    # Suma wystąpień dla każdej klasy
    class_counts = np.sum(all_labels, axis=0)
    # Formuła: total_examples / (num_labels * count)
    weights = total_samples / (num_labels * class_counts)
    # Zamieniamy na tensor z dtype float
    weights_tensor = torch.tensor(weights, dtype=torch.float)
    print("Obliczone wagi klas:", weights_tensor)
    return weights_tensor


# Definicja własnej klasy Trainer, która nadpisuje compute_loss i używa ważonej funkcji strat
class MyTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, num_items_in_batch=None, return_outputs=False):
        ...
        # Użycie BCEWithLogitsLoss z wagami dla klas
...
def train_roberta_multi_label(train_dataset, val_dataset, label_names):
    # Inicjalizujemy model – problem_type ustawiamy na multi_label_classification
    model = AutoModelForSequenceClassification.from_pretrained("roberta-base", num_labels=len(label_names),
    ...
    # Obliczamy wagi klas na podstawie zbioru treningowego
    class_weights = compute_class_weights(train_dataset, num_labels=len(label_names))

    # Używamy naszej niestandardowej klasy Trainer
    trainer = MyTrainer(
        model=model,
        args=training_args,
