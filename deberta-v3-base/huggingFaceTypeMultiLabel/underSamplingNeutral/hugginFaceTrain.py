"""
Train DeBERTa-v3-base on GoEmotions (multilabel) + pełny zestaw wykresów wynikowych.

Wyniki liczbowe zapisują się jak dotychczas do `results_goemotions/`, 
a wykresy pojawiają się w podkatalogu `results_goemotions/plots`.
"""
...
def plot_labels_per_sample(labels, save_path):
    """Histogram liczby etykiet przypisanych do przykładu."""
...
def plot_per_label_metrics(y_true, y_pred, label_names, save_path):
    """Bar chart F1, precision, recall per label."""
...
def plot_cooccurrence_heatmap(y_true, label_names, save_path):
    """Współwystępowanie etykiet (Jaccard)."""
    A = y_true.astype(bool)
    inter = (A.T @ A).astype(float)            # |A∩B|
    union = (A.sum(axis=0) + A.sum(axis=0)[:, None] - inter)  # |A∪B|
    with np.errstate(divide="ignore", invalid="ignore"):
        jaccard = np.where(union == 0, 0, inter / union)
    np.fill_diagonal(jaccard, np.nan)          # czytelniej bez przekątnej
...
def train_roberta_multi_label(train_dataset, val_dataset, label_names):
    ...
    # wszystkie wykresy zbudujemy tu:
    generate_all_plots(val_dataset["labels"], final_preds, probs,
                       label_names, best_threshold)

    return model


# ---------- 3.  Threshold tuning + wykres F1(th) ---------- #

def tune_threshold_and_plot(model, val_dataset, label_names,
    ...
    # zapis pojedynczych macierzy pomyłek
    ...
    return best_threshold, final_preds, probs


# ---------- 4.  Generowanie wszystkich wykresów ---------- #

def generate_all_plots(y_true, y_pred, probs, label_names, best_threshold):
    ensure_dir("results_goemotions/plots")
...
if __name__ == "__main__":
    ...
    _ = train_roberta_multi_label(train_ds, val_ds, label_names)
